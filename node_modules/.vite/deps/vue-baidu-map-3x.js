...
// node_modules/.pnpm/vue-baidu-map-3x@1.0.17_vue@3.2.36/node_modules/vue-baidu-map-3x/dist/vue-baidu-map-3x.es.js
var events = {
  "bm-map": [
    "click",
    "dblclick",
  ...
  watch: {
    ...
    "center.lng"(val, oldVal) {
      ...
      if (val !== oldVal && val >= -180 && val <= 180) {
        map.centerAndZoom(new BMap2.Point(val, center.lat), zoom);
      }
    },
    "center.lat"(val, oldVal) {
      const { BMap: BMap2, map, zoom, center } = this;
      if (val !== oldVal && val >= -74 && val <= 74) {
        map.centerAndZoom(new BMap2.Point(center.lng, val), zoom);
      }
    },
    zoom(val, oldVal) {
      const { map } = this;
...
var commonMixin = (type) => new Mixin({ type });
var _sfc_main$w = {
  name: "bm-scale",
  render() {
  },
...
var _sfc_main$v = {
  ...
  methods: {
    load() {
      ...
      this.originInstance = new BMap2[MethodMap[getConfig().type].NavigationControl](navigationOption);
      map.addControl(this.originInstance);
    }
  }
};
...
var _sfc_main$n = {
  ...
  methods: {
    load() {
      ...
      (_a = this.Cluster) == null ? void 0 : _a.addMaker(this);
    }
  },
  beforeUnmount() {
    var _a;
    (_a = this.Cluster) == null ? void 0 : _a.removeMaker(this);
  }
};
...
var _sfc_main$h = {
  ...
  watch: {
    ...
    "position.lng"(val, oldVal) {
      ...
      const lng = val;
      if (val.toString() !== oldVal.toString() && lng >= -180 && lng <= 180) {
        this.originInstance.setCenter(createPoint(BMap2, { lng, lat: this.center.lat }));
      }
    },
    "position.lat"(val, oldVal) {
      const { BMap: BMap2 } = this;
      const lat = val;
      if (val.toString() !== oldVal.toString() && lat >= -74 && lat <= 74) {
        this.originInstance.setCenter(createPoint(BMap2, { lng: this.center.lng, lat }));
      }
    },
    labelStyle: {
      handler(val) {
...
var _sfc_main$3 = {
  ...
  watch: {
    gridSize: {
      handler(val) {
        const { BMap: BMap2, originInstance } = this;
        originInstance.setGridSize(BMap2, val.map);
      },
      deep: true
    },
    maxZoom(val) {
    ...
    minClusterSize: {
      handler(val) {
        const { BMap: BMap2, originInstance } = this;
        originInstance.setMinClusterSize(createSize(BMap2, val));
      },
      deep: true
    },
    styles: {
  ...
  methods: {
    load() {
      ...
      this.originInstance = new MarkerClusterer$1(map, {
        gridSize: gridSize && createSize(BMap2, gridSize),
        maxZoom,
        minClusterSize: minClusterSize && createSize(BMap2, minClusterSize),
...
// dep:vue-baidu-map-3x
var vue_baidu_map_3x_default = index;
export {
  BaiduMap,
  BmAutoComplete,
  BmBoundary,
  ...
  vue_baidu_map_3x_default as default
};
